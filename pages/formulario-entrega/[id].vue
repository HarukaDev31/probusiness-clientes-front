...
function syncTabFromRoute () {
  const fromQuery = parseDestinoFromQuery(route.query.destino ?? route.query.tab)
...
/** Precarga catálogos del tab (sin red si ya están en memoria). */
...
watch(consolidadoId, () => loadConsolidadoBase(), { immediate: true })

watch(activeTab, (tab) => {
  if (tab) preloadTabCatalogs(tab)
}, { immediate: true })
</script>
