  ...
  <div :class="embedded ? '' : 'h-auto bg-gray-50 dark:bg-gray-900 py-8'">
    <div :class="embedded ? '' : 'max-w-7xl mx-auto px-4'">
      ...
      <div class="text-center mb-8">
        <h1 v-if="!embedded" class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Consolidado #{{ carga }}
        </h1>
...
const consolidadoId = props.consolidadoId
...
function applySessionDefaultsProvincia () {
...
function restoreLocalProvinciaState () {
...
async function loadUsuarioFacturacionProvincia (restoredFromPersistence: boolean) {
  try {
    const res = await getUsuarioDatosFacturacion('Provincia')
...
// Cargar datos iniciales (sin repetir red al volver al tab si ya se cargó)
onMounted(async () => {
  const id = Number(consolidadoId)

  if (provinciaFormBootstrapped.value) {
    await ensureProvinciaCatalogs(id)
    importadores.value = clientes.value
    const restored = restoreLocalProvinciaState()
    applySessionDefaultsProvincia()
    return
  }
  ...
  await ensureProvinciaCatalogs(id)
  importadores.value = clientes.value
  const restoredFromPersistence = restoreLocalProvinciaState()
  await loadUsuarioFacturacionProvincia(restoredFromPersistence)

  provinciaFormBootstrapped.value = true
  applySessionDefaultsProvincia()
})
</script>
